            ...
            if Phase.FIND_NEXT_GOAL==self._phase:
                ...
                if self._goalVic in self._foundVictims and self._goalVic not in self._uncarryable and 'location' in self._foundVictimLocs[self._goalVic].keys():
                    if self._foundVictimLocs[self._goalVic]['room'] in ['area A1', 'area A2', 'area A3', 'area A4'] and state[self.agent_id]['location'] in locs:
                        ...
                        I suggest you pick up this victim because it is located far away and you can move faster','RescueBot')
                ...
                if self._goalVic in self._foundVictims and 'location' not in self._foundVictimLocs[self._goalVic].keys():
                    self._sendMessage('Next victim to rescue: ' + self._goalVic ,'RescueBot')
                    self._sendMessage('Picking up ' + self._goalVic + ' in ' + self._foundVictimLocs[self._goalVic]['room'] + ' because it is the next victim to rescue and you found it there', 'RescueBot')
                    ...
                    for info in state.values():
                        if 'class_inheritance' in info and 'CollectableBlock' in info['class_inheritance']:
                            ...
                            if vic==self._goalVic and vic in self._uncarryable:
                                self._sendMessage('URGENT: You should pick up ' + vic + ' in ' + self._door['room_name'] + ' because I am not allowed to carry this victim', 'RescueBot')
                            ...
                            if vic==self._goalVic and vic in self._undistinguishable and vic not in self._foundVictims:
                                self._sendMessage('URGENT: You should clarify the gender of the injured baby in ' + self._door['room_name'] + ' because I cannot accurately distinguish between boy and girl', 'RescueBot')
                                self._foundVictim=str(info['img_name'][8:-4])
